...
def course_page(request):
    ...
    if filtered_course_date == applicant_course_date:
        ...
        try:
            certificate_name = certificates_list.objects.filter(name=applicant.name).filter(course_name = applicant.course_name).get()
            certs_link = True
        except certificates_list.DoesNotExist:
            certs_link = False
        context = {
...
def lecture_page(request, course_name):
    ...
    course_applied_start_date = '2020-06-23'
    print(course_name)
...
def generate_pdf(request):
    ...
    applicant = ApplicationModel.objects.filter(email=logged_in_user_email).first()
    try:
        certificate_name = certificates_list.objects.filter(name=applicant.name).filter(course_name = applicant.course_name)
        if certificate_name:
            context = {
            ...
            messages.info(request, 'We are sorry to inform you that you have not qualified the course due to incomplete attendance and/or work.')
